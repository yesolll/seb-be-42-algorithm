...
public class Uniform {
    public static int solution(int n, int[] lost, int[] reserve) {
        ...
        // 1. n + 2 크기의 체육복 수 배열 만들기 
        int[] uniforms = new int[n+2]; // 맨 앞, 맨 뒤는 index 에러 처리 없이 쉽게 비교하려고... 값은 0으로

        for (int i = 0; i < uniforms.length; i++) {
            uniforms[i] = 1;    // 기본 모든 애들 한 개씩
        }
        for (int i : lost) {
            uniforms[i] -= 1;   // 잃어버린 애들 -1개 처리
        }
        for (int i : reserve) {
            uniforms[i] += 1;   // 여벌있는 애들 +1개 처리
        }
        uniforms[0] = 0;
        ...
        // 2. 반복하면서 체육복 빌리기
        for (int i = 1; i <= n; i ++) {
            if (uniforms[i] == 0 && (uniforms[i - 1] == 2 || uniforms[i + 1] == 2)) { // if (나 0개 && (앞이 2개 || 뒤가 2개))
                uniforms[i]++;  // 나 ++;
                if (uniforms[i - 1] == 2) uniforms[i-1]--;       // if (앞이 2개) 앞--;
                else if (uniforms[i + 1] == 2) uniforms[i+1]--;  // else if (뒤가 2개) 뒤--;
            }
        }
